...
class Result:
    ...
    def calculate_result(self, y_true, y_score, fold=None, verbose=False):
        ...
        self.MCC.append(matthews_corrcoef(y_true, y_pred))
        self.G.append(geometric_mean_score(y_true, y_pred))
    ...
    def process(self, number):
        return np.round(np.median(number), 2)

    def show(self):
        print(
            f"{self.process(self.auc)}&{self.process(self.MCC)}&{self.process(self.G)} & {self.process(self.effectiveness)} & {self.process(self.f1_score_m)} & {self.process(self.precision_m)} & {self.process(self.recall_m)} & {self.process(self.f1_score_a)} & {self.process(self.precision_a)} & {self.process(self.recall_a)}")
